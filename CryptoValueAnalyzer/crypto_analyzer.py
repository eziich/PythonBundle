...
class CryptoAnalyzer:
    ...
    def setup_ui(self):
        ...
        # Fetch button
        self.fetch_btn = tk.Button(control_frame, text="📊 Fetch Top 10 Cryptocurrencies", 
    ...
    def _fetch_data_thread(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self.status_label.config(text="Fetching top 10 cryptocurrencies..."))
            ...
            for i, crypto in enumerate(crypto_list):
                self.root.after(0, lambda i=i, name=crypto['name']: self.status_label.config(
                    text=f"Fetching data for {name}... ({i+1}/10)"))
                ...
                try:
                    ...
                    print(f"Error fetching data for {crypto['name']}: {e}")
    ...
    def generate_predictions(self):
        """Generate price predictions for each cryptocurrency"""
        self.predictions = []
        
        for crypto in self.crypto_data:
            try:
                # Prepare data for prediction
                prices = np.array(crypto['prices']).reshape(-1, 1)
                ...
                current_price = crypto['current_price']
                predicted_7d = future_prices[-1][0]
    ...
    def update_charts(self):
        ...
        for crypto in self.crypto_data[:5]:  # Show top 5
